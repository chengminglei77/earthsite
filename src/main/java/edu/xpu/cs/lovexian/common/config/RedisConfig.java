...
public class RedisConfig extends CachingConfigurerSupport {
    ...
    @Value("${spring.redis.content-database:1}")
    private int contentDatabase;

    @Bean("contentJedisPool")
    public JedisPool contentJedisPoolFactory() {
        JedisPoolConfig jedisPoolConfig = new JedisPoolConfig();
        jedisPoolConfig.setMaxIdle(maxIdle);
        jedisPoolConfig.setMaxWaitMillis(maxWaitMillis);
        if (StringUtils.isNotBlank(password)) {
            return new JedisPool(jedisPoolConfig, host, port, timeout, password, contentDatabase);
        } else {
            return new JedisPool(jedisPoolConfig, host, port, timeout, null, contentDatabase);
        }
    }
